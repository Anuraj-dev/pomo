...
public object BackupCodec {
    ...
    public fun decode(json: String): PomoBackup? {
        ...
        val decoded =
            runCatching { plain.fromJson(root, PomoBackup::class.java) }.getOrNull()
                ?: return null
    ...
    private fun JsonObject.intOrNull(key: String): Int? = get(key)?.takeIf { it.isJsonPrimitive && it.asJsonPrimitive.isNumber }?.asInt

    private fun PomoBackup.sanitized(): PomoBackup =
        copy(
            history =
                BackupHistory(
            ...
            crew =
                crew.copy(
                    ...
                    dailyAggregates =
                        crew.dailyAggregates.filter {
                            it.crewId.isNotBlank() && it.identityPublicKey.isNotBlank() && it.localDate.isValidDate()
                        },
                    hiddenMembers =
                        crew.hiddenMembers.filter {
                            it.crewId.isNotBlank() && it.identityPublicKey.isNotBlank()
                        },
                ),
        )

    private fun BackupDayStats.isNonNegative(): Boolean = completed >= 0 && workMinutes >= 0 && breakMinutes >= 0
    ...
    private val KNOWN_PHASES: Set<String> =
        setOf(
