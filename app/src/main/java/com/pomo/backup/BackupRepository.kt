...
public class BackupRepository(context: Context) {
    ...
    public suspend fun export(): PomoBackup = withContext(Dispatchers.IO) {
        val memberships = crewStore.loadMemberships()
        PomoBackup(
            exportedAtEpochSeconds = System.currentTimeMillis() / 1000L,
            appVersionName = BuildConfig.VERSION_NAME,
            history = BackupHistory(
                dayStats = historyDao.getAllDayStatsSnapshot().map { day ->
                    BackupDayStats(
                        date = day.date,
                ...
                sessions = historyDao.getAllSessionsSnapshot().map { session ->
                    BackupSession(
                        start = session.start,
                        date = session.date,
                        type = session.type,
            ...
            crew = BackupCrew(
                ...
                snapshots = crewDao.getAllSnapshots().map { snapshot ->
                    BackupSnapshot(
                ...
                dailyAggregates = crewDao.getAllDailyAggregates().map { aggregate ->
                ...
                hiddenMembers = crewDao.getAllHiddenMembers().map { hidden ->
                    BackupHiddenMember(
    ...
    public suspend fun writeTo(uri: Uri): Boolean = withContext(Dispatchers.IO) {
    ...
    public suspend fun readFrom(uri: Uri): PomoBackup? = withContext(Dispatchers.IO) {
        runCatching {
    ...
    public suspend fun restore(backup: PomoBackup): BackupRestoreSummary = withContext(Dispatchers.IO) {
        ...
        val membershipsAdded = crewStore.mergeMemberships(
            memberships = backup.crew.memberships.map { membership ->
                CrewMembership(
                    crewId = membership.crewId,
                    crewName = membership.crewName,
                    joinCode = membership.joinCode,
        ...
        crewDao.restoreProjection(
            snapshots = backup.crew.snapshots.map { snapshot ->
                CrewSnapshotEntity(
                    crewId = snapshot.crewId,
                    identityPublicKey = snapshot.identityPublicKey,
                    displayName = snapshot.displayName,
            ...
            aggregates = backup.crew.dailyAggregates.map { aggregate ->
            ...
            hiddenMembers = backup.crew.hiddenMembers.map { hidden ->
