...
internal object SharedPreferencesMaterializer {
    private val sharedFields = setOf("focusMinutes", "shortBreakMinutes", "longBreakMinutes", "longBreakAfter", "defaultTagId")
    ...
    fun isDeviceLocal(field: String): Boolean =
        field in setOf("theme", "cueMode", "notificationPermission", "navigation", "routeHealth", "selectedCrew", "hiddenMembers")
}

internal data class ProfileVersion(val operationId: String, val name: String, val photoBlobId: String?)
internal data class ProfileProjection(val complete: ProfileVersion?, val pending: ProfileVersion?)
...
internal enum class MembershipIntent { JOIN, LEAVE }
internal data class CrewMembershipFact(val operationId: String, val crewId: String, val intent: MembershipIntent)
internal data class CrewMembershipProjection(val joined: Boolean?, val decisionRequired: Boolean, val publicationPaused: Boolean)
...
internal object CrewMembershipMaterializer {
    ...
    fun pseudonym(memberSecret: ByteArray, crewId: String): String {
        ...
        val digest = java.security.MessageDigest.getInstance("SHA-256").digest(memberSecret + crewId.toByteArray())
        return digest.joinToString("") { "%02x".format(it.toInt() and 0xff) }
    }
}

internal enum class DataFamilyDisposition { ACCEPTED, PENDING_FORWARD }
