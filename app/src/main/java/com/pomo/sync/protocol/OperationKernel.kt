...
internal fun interface OperationStore {
    /** Must atomically persist every wire, disposition, projection effect, and local outbox obligation. */
    fun commitBatch(commits: List<OperationCommit>)
    ...
    fun commit(
        ...
        commitBatch(
            listOf(OperationCommit(operation, disposition, localAuthor)) +
                reclassifications.map { OperationCommit(it.operation, it.disposition, false) },
        )
    }
...
internal class OperationKernel(
    ...
    fun author(request: AuthorRequest): AuthorResult {
        ...
        return runCatching {
            ...
            try {
                store.commit(authenticated, result.disposition, true, result.reclassifications)
            } catch (error: Throwable) {
                restoreState(before)
                throw error
            }
            record(result)
            AuthorResult.Authored(detachedCopy(authenticated), result.disposition)
    ...
    fun ingest(signedEnvelope: ByteArray): IngestDisposition {
        val authenticated =
            runCatching {
                verifier.verify(signedEnvelope)
            }.getOrElse { return reject(signedEnvelope, IngestDisposition.REJECTED_INVALID) }
        ...
        val result = ingestAuthenticated(authenticated)
        return try {
            store.commit(authenticated, result.disposition, false, result.reclassifications)
            record(result)
            result.disposition
        } catch (_: Throwable) {
            restoreState(before)
    ...
    private fun quarantineFork(
        ...
        val invalidatedCheckpointIds = feed.checkpointIds.filterKeys { it >= sequence }.values
        invalidatedCheckpointIds.forEach { quarantined += it }
        feed.checkpointIds.keys.removeAll { it >= sequence }
        if (invalidatedCheckpointIds.isNotEmpty()) checkpointPreferences.clear()
            ...
            val next =
                remaining.values
                    .filter { authenticated ->
                        val operation = authenticated.operation
                        val predecessorReady =
                            if (operation.sequence == 1L) {
                                operation.previousOperationId == null
                            } else {
                                operation.previousOperationId?.let { previous ->
                                    previous in emitted ||
                                        feeds[feedKey(operation.deviceId, operation.incarnationId)]
                                            ?.checkpointIds
                                            ?.get(operation.sequence - 1) == previous
                                } == true
                            }
                        predecessorReady &&
                            operation.frontier.all { dependency ->
                                val dependencyOperation =
                                    byPosition[
                    ...
                    .minWithOrNull { left, right ->
                        compareBytes(left.operationId.copy(), right.operationId.copy())
                    }
            checkNotNull(next) { "Accepted Operation graph contains an unresolved causal cycle" }
            remaining.remove(next.operationId)
            emitted += next.operationId
            ordered += next
        }
        return ordered
    }
