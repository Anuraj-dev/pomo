...
internal enum class RecoveryArtifactKind {
    AUTHORITY_FILE,
    DATA_ARCHIVE,
}

internal data class RecoveryArtifact(
    val kind: RecoveryArtifactKind,
    val salt: ByteArray,
    val nonce: ByteArray,
...
internal enum class RecoverySourceKind {
    DEVICE,
    MAILBOX,
    ARCHIVE,
}

internal data class ProvenancedRecoveryObject(
    val objectId: String,
    val sourceId: String,
    val sourceKind: RecoverySourceKind,
)
...
internal object RecoveryArtifactCodec {
        ...
        val payload =
            DeterministicCbor.encode(
                CborValue.Array(
                    listOf(
                        CborValue.Integer(1),
                        CborValue.Bytes(body.protectedAuthority),
                        CborValue.Integer(body.recoveryGeneration),
                        CborValue.Array(body.frontierEvidence.sorted().map(CborValue::Text)),
                        CborValue.Array(body.capabilityLocators.sorted().map(CborValue::Text)),
    ...
    fun validateArchiveManifest(manifest: RecoveryArchiveManifest, authorityGrantRequested: Boolean) {
        ...
        require(
            (manifest.checkpointIds + manifest.packIds + manifest.blobIds).distinct().size ==
    ...
    fun open(artifact: RecoveryArtifact, passphrase: String): ByteArray {
        ...
        return try {
            PomoCrypto.decryptAesGcm(
                key,
                AesGcmCiphertext(artifact.nonce, artifact.ciphertextAndTag),
                aad(artifact.kind),
            )
        } finally {
            key.fill(0)
...
internal enum class RestoreDomain {
    HISTORY,
    TAG,
    PREFERENCE,
    PROFILE,
...
internal data class RestoreSelection(
    val domain: RestoreDomain,
    val targetId: String,
    val compensatingPayload: ByteArray,
)

internal data class ForwardRestorePlan(
    val safetyCheckpointId: String,
    val compensating: List<RestoreSelection>,
    val independentConfirmationRequired: Boolean,
)

internal enum class RestorePlanOrigin {
    HUMAN,
    AGENT,
}

internal data class RestoreApproval(
    val humanConfirmed: Boolean,
    val independentConfirmed: Boolean,
)
...
internal fun compareHistoricalValues(
    left: HistoricalValue,
    right: HistoricalValue,
): Int {
...
internal fun prepareForwardRestore(
    ...
    val prohibited =
        setOf(
            RestoreDomain.ACTIVE_PHASE,
            RestoreDomain.DEVICE_KEY,
            RestoreDomain.CONTENT_EPOCH,
...
internal fun authorizeForwardRestore(
    plan: ForwardRestorePlan,
    origin: RestorePlanOrigin,
    destructive: Boolean,
