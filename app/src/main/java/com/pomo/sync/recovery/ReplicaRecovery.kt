...
internal data class FrontierHead(
    val feedKey: String,
    val sequence: Long,
...
internal data class JournalPack(
    val packId: String,
    val prefix: FrontierHead,
...
internal data class RecoverySource(
    val sourceId: String,
...
internal object Rehydrator {
    fun plan(sources: Collection<RecoverySource>): RehydrationPlan {
        ...
        operations.forEach { (_, copies) ->
            require(copies.map { it.wire.toList() }.distinct().size == 1) {
...
internal enum class IntegrityFailure {
    PROJECTION_CORRUPT,
    JOURNAL_CORRUPT,
    DEVICE_KEY_MISSING,
}

internal data class SafeModeState(
    val active: Boolean,
    val incarnationSealed: Boolean,
    val inspectionAllowed: Boolean,
