...
internal object HpkeP256 {
    ...
    private fun keySchedule(
        ...
        return try {
            val key = labeledExpand(secret, suiteId, "key", context, PomoSuite.HPKE_KEY_BYTES)
    ...
    private fun labeledExpand(
        ...
        require(length in 1..(255 * 32)) { "HKDF-SHA256 output length is out of range" }
        val labeledInfo =
    ...
    private fun dh(
        ...
        return try {
            ...
            ByteArray(32).also { secret.copyInto(it, 32 - secret.size) }
        } finally {
            secret.fill(0)
        }
    }
