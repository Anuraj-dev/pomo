...
internal object CoseSign1 {
    ...
    fun sign(
        ...
        require(operation.suite == PomoSuite.ID) { "Unsupported COSE signing suite" }
        require(operation.suiteGeneration == PomoSuite.INITIAL_GENERATION) {
            "Unsupported COSE signing suite generation"
        }
        val expectedCanonical = OperationCodec.encodeUnsigned(operation)
        require(expectedCanonical.contentEquals(canonicalUnsigned)) {
            "COSE signing payload must be the canonical Operation"
        }
