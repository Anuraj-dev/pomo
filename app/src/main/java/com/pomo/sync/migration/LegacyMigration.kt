...
internal data class LegacyInventoryItem(
    val sourceId: String,
...
internal data class MigrationInventory(
    val sourceId: String,
    val items: List<LegacyInventoryItem>,
    val durableItemCount: Int,
) {
    init {
...
internal enum class LegacyTimerState {
    PARKED,
    RUNNING,
    PAUSED,
}

internal data class MigrationIdentity(
    val memberId: String,
    val crewIds: Set<String>,
)

internal data class MigrationVerification(
    val expectedItems: Int,
    val explainedItems: Int,
...
internal fun requireIdentitySelection(sources: Collection<MigrationIdentity>, selectedMemberId: String?) {
    ...
    require(members.size <= 1 || selectedMemberId in members) {
        "Different Members require explicit selection and cannot be blended"
    }
    require(crews.size <= 1 || selectedMemberId != null) {
        "Different Crews require explicit selection and cannot be blended"
    }
}
...
internal fun verifyMigrationReady(prerequisites: MigrationPrerequisites) {
    ...
    require(prerequisites.verification.expectedItems == prerequisites.verification.explainedItems) {
        "zero unexplained omissions required"
    }
    require(prerequisites.verification.projectionRootsMatch && prerequisites.verification.domainInvariantsHold)
}

internal fun activateMigrationAtomically(
    prerequisites: MigrationPrerequisites,
    journalId: String,
    archiveId: String,
): MigrationActivation {
