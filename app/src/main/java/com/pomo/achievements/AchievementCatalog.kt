...
public object AchievementCatalog {
    ...
    public val all: List<Achievement> = listOf(
...
public object AchievementEvaluator {
    ...
    public fun earnedCount(snapshot: StatsSnapshot): Int =
        AchievementCatalog.all.count { it.isEarnedBy(snapshot) }
     ...
     */
    public fun earnedOnly(snapshot: StatsSnapshot): List<Achievement> =
        AchievementCatalog.all.filter { it.isEarnedBy(snapshot) }
    ...
    public fun highlights(snapshot: StatsSnapshot): List<Achievement> {
        val perAxis = listOf(AchievementAxis.Focus, AchievementAxis.Streak, AchievementAxis.BestDay)
            .mapNotNull { axis ->
