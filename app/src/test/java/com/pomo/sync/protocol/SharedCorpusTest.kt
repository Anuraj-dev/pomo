...
public class SharedCorpusTest {
    ...
    public fun operationCorpusProducesFrozenCanonicalIdentity() {
        ...
        val kernel =
            OperationKernel(
                ...
                CoseKernelVerifier { deviceId ->
                    if (deviceId == operation.deviceId) signingPublicKey else null
                },
                OperationStore { },
                CheckpointVerifier { },
            )
        ...
        val expectedSummary = fixture.getAsJsonObject("expected").getAsJsonObject("summary")
        val summary = kernel.summarize()
        assertEquals(expectedSummary.int("accepted"), summary.accepted)
        assertEquals(expectedSummary.int("pending"), summary.pending)
        assertEquals(expectedSummary.int("quarantined"), summary.quarantined)
        // Rejected-operation durability belongs to #103; the #102 tracer corpus fixes this at zero.
        assertEquals(0, expectedSummary.int("rejected"))
    }
    ...
    public fun negativeCorpusFailsClosedOnAndroid() {
        ...
        val nonCanonicalCases =
            cases.filter {
                it.asJsonObject.string("expected") == "REJECT_NON_CANONICAL" &&
                    it.asJsonObject.has("inputHex")
            }
        assertTrue(nonCanonicalCases.isNotEmpty())
        nonCanonicalCases.forEach { fixture ->
            assertThrows(IllegalArgumentException::class.java) {
                DeterministicCbor.decodeCanonical(fixture.asJsonObject.hex("inputHex"))
            }
        }
        val unsupportedCases =
            cases.filter {
        ...
        assertTrue(unsupportedCases.isNotEmpty())
        unsupportedCases.forEach { fixture ->
            assertThrows(IllegalArgumentException::class.java) {
        ...
        assertTrue(signatureCases.isNotEmpty())
        signatureCases.forEach { fixture ->
            assertThrows(IllegalArgumentException::class.java) {
                PomoCrypto.verifyP256LowS(
                    publicKey,
        ...
        val argonProfileCases = cases.filter { it.asJsonObject.has("parameters") }
        assertTrue(argonProfileCases.isNotEmpty())
        argonProfileCases.forEach { fixture ->
