...
public class DiagnosticExportTest {
    @Test public fun aliasesReferencesAndNeverExportsStableIds() {
        val output = ByteArrayOutputStream()
        DiagnosticExporter.export(sequenceOf(DiagnosticEvent(1, EvidenceArea.FRONTIER, "advanced", mapOf("sequence" to "3"), "stable-device", "stable-operation")), output)
    ...
    @Test public fun rejectsPlaintextAndSupportsCancellation() {
        assertTrue(runCatching { DiagnosticRedaction.requireSafe(DiagnosticEvent(1, EvidenceArea.MIGRATION, "event", mapOf("outcome" to "profile content"))) }.isFailure)
        val result = DiagnosticExporter.export(generateSequence { DiagnosticEvent(1, EvidenceArea.PERFORMANCE, "sample", mapOf("durationMs" to "1")) }, ByteArrayOutputStream()) { true }
        assertTrue(result.cancelled)
    }
    @Test public fun recorderIsBounded() {
