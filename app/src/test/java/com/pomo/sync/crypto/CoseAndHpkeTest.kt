...
public class CoseAndHpkeTest {
    ...
    public fun coseSign1BindsProtectedProfilePayloadAndDevice() {
        ...
        assertThrowsAny { CoseSign1.sign(operation.copy(suite = 2), canonical, pair.private) }
        assertThrowsAny {
            CoseSign1.sign(
                operation.copy(suiteGeneration = PomoSuite.INITIAL_GENERATION + 1),
                canonical,
                pair.private,
            )
        }
        assertThrowsAny { CoseSign1.sign(operation, canonical + byteArrayOf(0), pair.private) }
