...
public class LegacyMigrationTest {
    private val ready = MigrationPrerequisites("anchor", true, LegacyTimerState.PARKED, true, MigrationVerification(2, 2, true, true))

    @Test public fun inventoryRequiresDispositionForEveryDurableItem() {
        val kinds = LegacyDisposition.entries.mapIndexed { index, kind -> LegacyInventoryItem("android", "id-$index", "history", kind, "classified") }
    ...
    @Test public fun identityBaselineTimerAndOmissionsBlockCutover() {
        assertTrue(runCatching { requireIdentitySelection(listOf(MigrationIdentity("a", setOf("crew-a")), MigrationIdentity("b", setOf("crew-b"))), null) }.isFailure)
    ...
    @Test public fun atomicSuccessSealsArchiveAndPermanentlyRetiresDualWrite() {
