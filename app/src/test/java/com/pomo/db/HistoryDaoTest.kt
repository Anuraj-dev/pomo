...
public class HistoryDaoTest {
    ...
    @Test
    public fun insertAndRead_dayStats_roundTrip(): Unit =
        runTest {
    ...
    @Test
    public fun getDayStats_missingKey_returnsNull(): Unit =
        runTest {
            assertNull(dao.getDayStats("9999-01-01"))
        }

    @Test
    public fun replaceAllHistory_clearsAndRepopulates(): Unit =
        runTest {
    ...
    @Test
    public fun getSessionsForDates_returnsOnlyRequestedDatesInExportOrder(): Unit =
        runTest {
    ...
    @Test
    public fun aggregates_returnExpectedSums(): Unit =
        runTest {
            dao.insertAllDayStats(
                listOf(
    ...
    @Test
    public fun completedCountForDate_countsOnlyWorkAndCompleted(): Unit =
        runTest {
            ...
            dao.insertAllSessions(
                listOf(
                    ...
                    // not counted
                    SessionEntity(3L, "2026-05-07", "work", 1500, completed = false),
                    // not work
                    SessionEntity(4L, "2026-05-07", "short", 300, completed = true),
                    // not work
                    SessionEntity(5L, "2026-05-07", "long", 900, completed = true),
                ),
            )
    ...
    @Test
    public fun unsynced_returnsOnlySyncedFalse(): Unit =
        runTest {
            dao.insertDayStats(DayStatsEntity("2026-05-07", 0, 0, 0))
            dao.insertAllSessions(
                listOf(
    ...
    @Test
    public fun markAsSynced_flipsFlag(): Unit =
        runTest {
            dao.insertDayStats(DayStatsEntity("2026-05-07", 0, 0, 0))
            dao.insertAllSessions(
                listOf(
