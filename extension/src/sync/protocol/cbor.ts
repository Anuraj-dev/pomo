...
export function encodeCanonicalCbor(value: CborValue): Uint8Array {
  ...
  if (typeof value === "string") {
    if (hasLoneSurrogate(value)) throw new Error("CBOR text must contain valid Unicode scalar values");
  ...
  if (Array.isArray(value)) {
    if (value.length > MAX_COLLECTION_ITEMS) throw new Error("CBOR collection exceeds limit");
    return concat([head(4, value.length), ...value.map(encodeCanonicalCbor)]);
  }
  if (value instanceof Map) {
    if (value.size > MAX_COLLECTION_ITEMS) throw new Error("CBOR collection exceeds limit");
...
class Decoder {
  ...
  #item(depth: number): CborValue {
    ...
    if ((major === 4 || major === 5) && argument > MAX_COLLECTION_ITEMS) throw new Error("CBOR collection exceeds limit");
