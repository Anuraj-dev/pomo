...
export async function deriveRecoveryKey(
  ...
  if (passphrase.length === 0) throw new Error("recovery passphrase must not be empty");
  if (hasLoneSurrogate(passphrase)) throw new Error("recovery passphrase must contain valid Unicode scalar values");
  if (passphrase.normalize("NFC") !== passphrase) throw new Error("recovery passphrase must be NFC-normalized");
