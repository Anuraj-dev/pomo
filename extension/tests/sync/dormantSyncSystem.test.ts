...
test("test artifact uses authenticated ingress while production stays dormant", async () => {
  ...
  const system = new DormantSyncSystem({ async ingest(wire) { received = wire; return "ACCEPTED"; } }, { productionActivated: false, testArtifact: true });
  ...
  expect(received).toEqual(new Uint8Array([1, 2, 3])); expect(system.productionMigrationCutoverAllowed()).toBeFalse();
