...
const nonCanonicalNegatives = negatives.cases.filter((item) => item.expected === "REJECT_NON_CANONICAL" && item.inputHex !== undefined);
...
describe("strict deterministic CBOR", () => {
  ...
  test("shared non-canonical category is nonempty", () => {
    expect(nonCanonicalNegatives.length).toBeGreaterThan(0);
  });

  for (const fixture of nonCanonicalNegatives) {
  ...
  test("rejects lone surrogates and oversized encoded collections", () => {
    expect(() => encodeCanonicalCbor("\ud800")).toThrow(/Unicode/);
    expect(() => encodeCanonicalCbor(Array.from({ length: 4097 }, () => null))).toThrow(/collection/);
    expect(() => encodeCanonicalCbor(new Map(Array.from({ length: 4097 }, (_, index) => [index, null] as const)))).toThrow(/collection/);
  });
});
