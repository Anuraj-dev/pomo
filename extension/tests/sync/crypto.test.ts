...
const rawSignatureNegatives = negatives.cases.filter((item) => item.rawSignatureHex !== undefined);
const argonProfileNegatives = negatives.cases.filter((item) => item.parameters !== undefined);
...
describe("POMO-SUITE-1 shared primitive corpus", () => {
  test("shared primitive negative categories are nonempty", () => {
    expect(rawSignatureNegatives.length).toBeGreaterThan(0);
    expect(argonProfileNegatives.length).toBeGreaterThan(0);
  });

  primitives.hkdfSha256.forEach((fixture, index) => {
    test(`matches RFC 5869 HKDF-SHA256 vector ${index}`, async () => {
  ...
  primitives.aes256Gcm.forEach((fixture, index) => {
    test(`matches NIST AES-256-GCM vector ${index}`, async () => {
  ...
  primitives.ecdsaP256Sha256.forEach((fixture, index) => {
    test(`accepts shared low-S ESP256 vector ${index} and rejects alternate wire shapes`, async () => {
      ...
      for (const negative of rawSignatureNegatives) {
        expect(await verifyP256LowS(publicKey, utf8ToBytes(fixture.messageUtf8), hexToBytes(negative.rawSignatureHex!))).toBe(false);
      }
    });
  });

  primitives.hpkeP256.forEach((fixture, index) => {
    test(`matches RFC 9180 P-256 Base-mode sequence-zero vector ${index}`, async () => {
  ...
  test("matches the shared exact Argon2id recovery profile and rejects parameter drift", async () => {
    ...
    for (const negative of argonProfileNegatives) {
      expect(() => assertRecoveryArgon2idProfile(negative.parameters!)).toThrow(/profile/);
    }
  });

  test("rejects passphrases outside the canonical recovery text boundary", async () => {
    const salt = hexToBytes(primitives.pomoRecoveryArgon2idProfile.saltHex);
    await expect(deriveRecoveryKey("", salt)).rejects.toThrow(/empty/);
    await expect(deriveRecoveryKey("e\u0301", salt)).rejects.toThrow(/NFC/);
    await expect(deriveRecoveryKey("\ud800", salt)).rejects.toThrow(/Unicode/);
  });
});
